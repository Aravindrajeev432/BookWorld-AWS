...
def product_view(request, st, bname):
    ...
    try:
        in_cart = CartItem.objects.filter(
            ...
            product_id=p_id).exists()
    except BaseException:
        raise Http404()
    if not in_cart:
        try:
            in_cart = CartItem.objects.filter(
                Q(product_id=p_id) & Q(user=user)).exists()
        except BaseException:
            in_cart = False
    print(in_cart)
    try:
...
def cat_view(request, slug):
    ...
    product = Product.objects.filter(
        category_id=cat_id, is_active=True).defer(
        'description', 'is_active').select_related("category").all()
    cat = Category.objects.all()
...
def search(request):
    ...
    if 'search' in request.GET:
        ...
        if keyword:
            try:
                ...
                bannerimg = ""
            pro = Product.objects.order_by('book_name').filter(Q(book_name__icontains=keyword) | Q(
                author__icontains=keyword), is_active=True).order_by('id')
    ...
    return render(
        request, 'landing.html', {
            'pro': paged_products, 'category': cat, 'bannerimg': bannerimg, })
