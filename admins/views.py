...
def dashboard(request):
    ...
    order_graph = Order.objects.filter(is_ordered=True).values(
        'created_at__date').order_by('-created_at__date').annotate(sum=Sum('order_total'))[9:]

    order_status_graph = OrderProduct.objects.filter().values(
        'status').annotate(count=Count('status'))
    order_product_count_graph = OrderProduct.objects.filter().values(
        'quantity').order_by('created_at__date')[:7].annotate(count=Count('quantity'))
...
def products(request):
    ...
    product = Product.objects.all().order_by(
        'book_name').select_related('category')
    if request.method == 'POST':
        s = request.POST['search']
        product = Product.objects.filter(
            book_name__icontains=s).select_related('category')
    ...
    return render(request,
                  'admin/productmanagement.html',
                  {'product': product,
                   'p_count': p_count,
                   'outofstock': outofstock,
                   'pro': pro,
                   'book_selled_count': book_selled_count,
...
def edit_book(request, id):
    ...
    return render(request, 'admin/edit_book.html',
                  {'book_details': book_details, 'category': category})
...
def addnewbooktest(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Successfuly Added')
    cat = Category.objects.only('category_name')
    return render(request, 'admin/addnewbooktest.html',
                  {'cat': cat, 'form': form})
...
def users(request):
    ...
    no_books_buyed = OrderProduct.objects.values(
        'user_id').annotate(sum=Sum('quantity'))

    no_books_buyed = list(no_books_buyed)

    total_money_spent = Order.objects.values(
        'user_id').annotate(sum=Sum('order_total'))
    ...
    if request.method == 'POST':
        ...
        accounts = paginator.get_page(page)
        return render(request,
                      'admin/usersview.html',
                      {'account': accounts,
                       'usercount': usercount,
                       'blocked_users_count': blocked_users_count,
                       'active_users_count': active_users_count,
                       'no_books_buyed': no_books_buyed,
    ...
    if 'search' in request.GET:
        ...
        if keyword:
            ...
            if keyword == "SelectAll":
                order_details = Order.objects.all().filter(
                    ~Q(is_ordered=False)).order_by('-created_at')
                p = Paginator(order_details, 5)
                page = request.GET.get('page', 1)
                order_details_paginated = p.get_page(page)
                total_orders_count = OrderProduct.objects.all().count()

                orders_pending = OrderProduct.objects.filter(
                    status__contains='Processing').count()
                orders_deliverd = OrderProduct.objects.filter(
                    status__contains='Deliverd').count()
                ...
                order_details = Order.objects.all().filter(~Q(is_ordered=False) & Q(
                    payment__payment_method__contains=keyword)).order_by('-created_at')
                p = Paginator(order_details, 5)
                page = request.GET.get('page', 1)
                order_details_paginated = p.get_page(page)
                total_orders_count = OrderProduct.objects.all().count()
        ...
        order_details = Order.objects.all().filter(
            ~Q(is_ordered=False)).select_related('user','payment').order_by('-created_at')
        p = Paginator(order_details, 5)
        page = request.GET.get('page', 1)
        order_details_paginated = p.get_page(page)
        total_orders_count = OrderProduct.objects.all().count()
...
def change_order_status(request, order_number, status):
    ...
    OrderProduct.objects.filter(
        order_id=order_product_details.id).update(
        status=status, updated_at=x)
...
def orders_page(request, order_number):
    ...
    order_product_details = OrderProduct.objects.filter(order_id=order_id).select_related('product')
...
def invoice(request, order_number):
    ...
    net_qty = OrderProduct.objects.filter(
        order_id=order_id).aggregate(
        Sum('quantity'))
    print(net_qty)
    print(order_product_details)
...
def pdf_report_create(request):
    ...
    dates_date = OrderProduct.objects.values(
        'created_at__date').distinct().order_by('created_at__date')
    ...
    try:
        sales = OrderProduct.objects.filter(
            created_at__date=dates[-1]).values('product_id').annotate(qty=Sum('quantity'))
    except BaseException:
        sales = []
    # get total money earned in day qty*productprice
        try:
            total_earn = Payment.objects.filter(
                created_at=dates[-1]).aggregate(Sum('amount_paid'))
        except BaseException:
            pass
    try:
    ...
    if request.method == "POST":
        try:
            ...
            sales = OrderProduct.objects.filter(
                created_at__date=salesdate).values('product_id').annotate(
                qty=Sum('quantity'))
            print(sales)
            for s in sales:
                pass
        ...
        try:
            grand_total_after_discount = Payment.objects.filter(
                created_at=salesdate).aggregate(Sum('amount_paid'))
        except BaseException:
...
def export_excel(request):
    ...
    response['Content-Disposition'] = 'attachement; filename=SalesReport' + \
    ...
    rows = Order.objects.filter(Q(created_at__date=salesdate) & Q(is_ordered=True)).values_list(
        'order_number', 'first_name', 'order_total', 'created_at__date')
...
def export_excel_year(request):
    ...
    rows = Order.objects.filter(
        Q(
            created_at__year=y) & Q(
            created_at__month=m) & Q(
                is_ordered=True)).values_list(
                    'order_number',
                    'first_name',
                    'order_total',
        'created_at__date')
...
def offer_management_productsview(request):
    ...
    offerset = Product.objects.raw(
        'select * from store_product LEFT JOIN store_product_offer on store_product.id=store_product_offer.product_id;')
    print(offerset)
    for of in offerset:
        print(of.book_name)
        print(of.discount)
    ...
    return render(
        request,
        'admin/offers/offer_management_productsview.html',
        context)
...
def offer_management_product(request, bid):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if offer_active == "true":
                offerproduct = Product_Offer(
                    product=book_details, discount=discount_rate, active=True)
                offerproduct.save()
            else:
                offerproduct = Product_Offer(
                    product=book_details, discount=discount_rate, active=False)
                offerproduct.save()
            return JsonResponse(
                {
                    'success': True},
    ...
    return render(
        request,
        'admin/offers/offer_management_product.html',
        context)
...
def offer_management_categoryview(request):
    ...
    if user.is_active:
        if not user.is_admin:
            return redirect('/')
    offerset = Category.objects.raw(
        'SELECT * from category_category LEFT join store_category_offer ON store_category_offer.category_id = category_category.id')
    category = Category.objects.all()
    context = {
        'category': offerset,
    }
    return render(
        request,
        'admin/offers/offer_management_categoryview.html',
        context)
...
def offer_management_category(request, cid):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if offer_active == "true":
                offerproduct = Category_Offer(
                    category=cat_details, discount=discount_rate, active=True)
                offerproduct.save()
            else:
                offerproduct = Category_Offer(
                    category=cat_details, discount=discount_rate, active=False)
                offerproduct.save()
    context = {
    ...
    return render(
        request,
        'admin/offers/offer_management_category.html',
        context)
...
def returns(request):
    ...
    returnproduct = OrderProduct.objects.raw(
        'SELECT *,orders_return_products.id AS returnid FROM orders_orderproduct  join orders_return_products ON orders_return_products.return_product_id = orders_orderproduct.id')
...
def return_order_admin(request, opid):
    ...
    if request.method == 'POST':
        change_order_product_status = OrderProduct.objects.get(
            id=return_product_details.return_product_id)
...
def offer_management_couponview(request):
    ...
    return render(
        request,
        'admin/offers/offer_management_couponview.html',
        context)
...
def coupon_view(request, id):
    ...
    return render(
        request,
        'admin/offers/offer_management_coupon_details.html',
        context)
