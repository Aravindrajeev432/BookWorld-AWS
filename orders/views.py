        ...
        if payment_method == "Paypal":
            ...
            if form.is_valid():
                ...
                order = Order.objects.get(
                    user_id=uid, is_ordered=False, order_number=order_number)
                ...
                return render(
                    request,
                    'store/review_checkout_paypal.html',
                    context)
            ...
            if form.is_valid():
                ...
                return render(
                    request,
                    'store/review_checkout_cod.html',
                    context)
            ...
            if form.is_valid():
                ...
                return render(
                    request,
                    'store/review_checkout_razorpay.html',
                    context)
...
def order_history(request):
    ...
    if 'email' in request.session:
        ...
        returndate = {}
        order_history = OrderProduct.objects.filter(
            user_id=uid).annotate(
            ret=F('updated_at__date') -
            current_date).order_by('-created_at').select_related(
            'order',
            'product',
            'payment')
...
def return_order(request, oid):
    ...
    if OrderProduct.objects.filter(Q(id=oid) & Q(user=uid)).exists():
        ...
        return_details = ''
        if Return_Products.objects.filter(
                return_product=order_details.id).exists():
            print("redirect")
            is_in_return = True
            return_details = Return_Products.objects.get(
                return_product=order_details.id)
        ...
        if request.method == "POST":
            ...
            if reson == "1":
                ...
                ret = Return_Products(
                    return_product=order_details,
                    returnstatus='Waiting',
                    reson="Damaged Product",
                    comment=comment)
                ret.save()
            elif reson == "2":
                ...
                ret = Return_Products(
                    return_product=order_details,
                    returnstatus='Waiting',
                    reson="Didn't meet my expectations",
                    comment=comment)
                ret.save()
            else:
                print("##")
...
def orders_page(request, order_number):
    ...
    order_product_details = OrderProduct.objects.filter(
