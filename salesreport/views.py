...
def salespage(request, *args, **kwargs):
    ...
    try:
        ...
        grandtotalfind = OrderProduct.objects.filter(
            created_at__date=dates[-1]).all().select_related('product')

        total_without_discount = 0

        total_after_coupon_discount = 0
        total_after_coupon = Order.objects.filter(
            created_at__date=dates[-1]).all()
    ...
    if request.method == "POST":
        try:
            ...
            sales = OrderProduct.objects.filter(
                ...
                qty=Sum('quantity'))
            grandtotalfind = OrderProduct.objects.filter(
                created_at__date=salesdate).all()
        ...
        try:
            ...
            total = "calculating"
    available_years = OrderProduct.objects.values(
        'created_at__year').distinct().order_by('created_at__year')
    context = {
        'dates': dates,
        'dates_max': dates_max,
        ...
        if 'month' in request.GET:
            ...
            try:
                if m == "0":
                    ...
                    sales = OrderProduct.objects.filter(Q(created_at__year=y) & Q(
                        ordered=True)).values('product_id').annotate(qty=Sum('quantity'))
                    try:
                        total_earn = Payment.objects.filter(
                            created_at__year=y).all()
                        total = 0
                        for t in total_earn:
                            total += float(t.amount_paid)
                        ...
                        total = "calculating"
                    grandtotalfind = OrderProduct.objects.filter(
                        created_at__year=y).all().select_related('product')

                    total_without_discount = 0
                    total_after_coupon_discount = 0
                    total_after_coupon = Order.objects.filter(
                        created_at__year=y).all().select_related('coupon')
                    for t_a_c in total_after_coupon:
                        ...
                        try:
                            ...
                            pass
                    for t in grandtotalfind:
                        total_without_discount += (t.product.price) * \
                            (t.quantity)

                else:

                    sales = OrderProduct.objects.filter(Q(created_at__year=y) & Q(ordered=True) & Q(
                        created_at__month=m)).values('product_id').annotate(qty=Sum('quantity'))
                    try:
                        total_earn = Payment.objects.filter(
                            Q(created_at__year=y) & Q(created_at__month=m)).all()
                        total = 0
                        for t in total_earn:
                            total += float(t.amount_paid)
                        print("total")
                        ...
                        total = "calculating"
                    grandtotalfind = OrderProduct.objects.filter(
                        Q(created_at__year=y) & Q(created_at__month=m)).all().select_related('product')
                    total_without_discount = 0
                    total_after_coupon_discount = 0
                    total_after_coupon = Order.objects.filter(
                        Q(created_at__year=y) & Q(created_at__month=m)).all().select_related('coupon')
                    for t_a_c in total_after_coupon:
        ...
        if m is not None:
            ...
            try:
                if m == "0":
                    ...
                    sales = OrderProduct.objects.filter(Q(created_at__year=y)).values(
                        'product_id').annotate(qty=Sum('quantity'))
                    try:
                        total_earn = Payment.objects.filter(
                            created_at__year=y).all()
                        total = 0
